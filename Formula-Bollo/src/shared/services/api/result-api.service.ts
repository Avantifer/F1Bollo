...
export class ResultApiService {
  ...
  getAllResultsPerCircuit(circuitId: number, seasonNumber?: number): Observable<Result[]> {
  ...
  saveResults(results: Result[], seasonNumber?: number): Observable<string> {
    const params = seasonNumber ? new HttpParams().set('circuitId', seasonNumber) : undefined;
    const headers = new HttpHeaders({
      'Content-Type': 'application/json',
      'Accept': 'text/plain'
    });
    return this.httpClient.put<string>(environment.apiUrl + this.endpoint + "/save", results, {headers, params, responseType: 'text' as 'json'});
  }
}
