...
export class FantasyTeamComponent {
  ...
  */
  changeOptionSelected(event: EventTarget | null): void {
    let previousOptionSelected: Element | null = document.querySelector(".fantasySelection-mid-table-top-option.selected");
    if (previousOptionSelected) previousOptionSelected.classList.remove('selected');
  ...
  */
  private changeOptionFormSelected() {
    let elementPreviousSelected: Element | null = document.querySelector('.fantasySelection-optionForm.selected');
  ...
  */
  changeDriverTeamSelected(event: EventTarget | null): void {
    if (event === null) return;
    let previousOptionFormSelected: Element | null = document.querySelector(".fantasySelection-optionForm.selected");
    if (previousOptionFormSelected) previousOptionFormSelected.classList.remove('selected');
  ...
  */
  addDriverToSelection(driver: FantasyPriceDriver): void {
    let optionFormSelected:  Element | null = document.querySelector(".fantasySelection-optionForm.selected");
  ...
  */
  private comprobateOptionFantasySelection(): void {
    ...
    let optionSelected: NodeListOf<Element> | null = document.querySelectorAll(".fantasySelection-mid-table-top-option");
  ...
  */
  private setOrderDriver(driver: FantasyPriceDriver, element: Element): void {
    if (element.classList[1].includes('team')) {
      this.messageService.showInformation('No puedes poner un piloto como equipo');
      return;
    }
  ...
  */
  private putDriverSelected(element: Element): void {
    ...
    if (!this.fantasyElection.driverOne) {
      firstContainerNotPreviousSelected = element.parentElement?.parentElement?.childNodes.item(0) as Element;
    } else if (!this.fantasyElection.driverTwo) {
      firstContainerNotPreviousSelected = element.parentElement?.parentElement?.childNodes.item(1) as Element;
    } else if (!this.fantasyElection.driverThree) {
      firstContainerNotPreviousSelected = element.parentElement?.parentElement?.childNodes.item(2) as Element;
    }
  ...
  */
  private setOrderTeam(team: FantasyPriceTeam, element: Element): void {
    if (element.classList[1].includes('driver')) {
      this.messageService.showInformation('No puedes poner un equipo como piloto');
      return;
    }
  ...
  */
  comprobateIfTeamIsElected(team: FantasyPriceTeam): boolean {
    ...
    if (this.fantasyElection.teamOne?.id === team.team.id || this.fantasyElection.teamTwo?.id === team.team.id) {
      notSelected = false;
    }
  ...
  */
  removeDriverSelected(event: EventTarget | null): void {
    ...
    let element: Element | null = event as Element;
    ...
    let input: NullableElement = element.parentElement?.nextElementSibling;
    let previousInputSelected: Element | null = document.querySelector('.fantasySelection-optionForm.selected');
  ...
  */
  private deleteDriverOfTheSelection(element: Element) {
    let driverNumberClass: string | null | undefined = element.parentElement?.nextElementSibling?.classList.item(1)
  ...
  */
  private deleteDriverTeamBackground(element: Element) {
    let elementPhoto: NullableElement = element.parentElement?.nextElementSibling?.nextElementSibling
    if (elementPhoto === null || elementPhoto === undefined) return;
    elementPhoto.classList.remove(elementPhoto.classList.item(elementPhoto.classList.length - 1)!);
  }
  ...
  */
  private calculateDriversValue(): void {
    if (this.fantasyElection.driverOne) {
      let driverOne: FantasyPriceDriver = this.driversList.filter((driver: FantasyPriceDriver) => this.fantasyElection.driverOne!.id === driver.driver.id)[0];
      this.price -= driverOne?.price;
    }

    if (this.fantasyElection.driverTwo) {
      let driverTwo: FantasyPriceDriver = this.driversList.filter((driver: FantasyPriceDriver) => this.fantasyElection.driverTwo!.id === driver.driver.id)[0];
      this.price -= driverTwo?.price;
    }

    if (this.fantasyElection.driverThree) {
      let driverThree: FantasyPriceDriver = this.driversList.filter((driver: FantasyPriceDriver) => this.fantasyElection.driverThree!.id === driver.driver.id)[0];
  ...
  */
  private calculateTeamsValue(): void {
    if (this.fantasyElection.teamOne) {
      let teamOne: FantasyPriceTeam = this.teamsList.filter((team: FantasyPriceTeam) => this.fantasyElection.teamOne!.id === team.team.id)[0];
      this.price -= teamOne?.price;
    }

    if (this.fantasyElection.teamTwo) {
      let teamTwo: FantasyPriceTeam = this.teamsList.filter((team: FantasyPriceTeam) => this.fantasyElection.teamTwo!.id === team.team.id)[0];
  ...
  */
  saveFantasyElection(): void {
    if (
      this.fantasyElection.driverOne && this.fantasyElection.driverTwo && this.fantasyElection.driverThree
      && this.fantasyElection.teamOne && this.fantasyElection.teamTwo
    ) {
      if (this.price < 0) return this.messageService.showInformation('No puedes tener dinero en negativo');

      this.fantasyElection.race = this.raceSelected;
      this.fantasyElection.season = this.raceSelected?.season;
      this.fantasyElection.user = new User(parseInt(this.authJWTService.getIdFromToken(localStorage.getItem('auth')!))) ;

      this.fantasyApiService.saveFantasyElection(this.fantasyElection)
        .pipe(
          takeUntil(this._unsubscribe)
        ).subscribe({
          next: (success: string) => {
      ...
      this.messageService.showInformation('Tienes que rellenar tu equipo entero');
    }
  }
  ...
  */
  getFantasyElection(): void {
    this.fantasyApiService.getFantasyElection(this.raceSelected!.id, parseInt(this.authJWTService.getIdFromToken(localStorage.getItem('auth')!)))
      .pipe(
        takeUntil(this._unsubscribe)
      )
      .subscribe({
